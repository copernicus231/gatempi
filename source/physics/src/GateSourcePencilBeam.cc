...
//Gaussian distribution generation for direction
  mEngineXTheta = new HepJamesRandom(); mEngineYPhi = new HepJamesRandom();
...
//Gaussian distribution generation for energy
  mEngineEnergy = new HepJamesRandom();
//Others
  mTestFlag=false;
...
mGaussianEnergy = new RandGauss(mEngineEnergy, mEnergy, mSigmaEnergy);
...
mGaussian2DXTheta = new RandMultiGauss(mEngineXTheta,mUXTheta,mSXTheta);
...
mGaussian2DYPhi = new RandMultiGauss(mEngineYPhi,mUYPhi,mSYPhi);
