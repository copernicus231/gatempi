              ...
              if( partDef )
                {
                  if( !( partDef->GetPDGStable() ) )
                    {
                      G4double lifeTime = partDef->GetPDGLifeTime();
                      if( nVerboseLevel > 0 )
                        G4cout << "GateVSource::GetNextTime : PDG lifetime (s) " 
                               << lifeTime/s << G4endl;
                      activityNow = m_activity *
                        exp( - ( timeStart - m_startTime ) / lifeTime );
                    }
                  else
                    if( nVerboseLevel > 1 ) 
  ...
  if ( test == 1  ) 
    {
      if (GetType() == G4String("backtoback"))    { GeneratePrimariesForBackToBackSource(event); }
        ...
        // if the command /gate/decay/setPrimaryDecayTimeGeneration 0 is issued
